...
public_users.get('/server/asynbooks/isbn/:isbn', function (req, res) {
  let { isbn } = req.params;
  axios
    .get(`http://localhost:5005/isbn/${isbn}`)
    ...
    .catch(function (error) {
      ...
      return res
        .status(500)
        .json({ message: 'Error while fetching book details.' });
    });
});
...
public_users.get('/server/asynbooks/author/:author', function (req, res) {
  ...
  axios
    .get(`http://localhost:5005/author/${author}`)
    .then(function (response) {
      console.log(response.data);
      return res.status(200).json(response.data);
    })
...
public_users.get('/server/asynbooks/title/:title', function (req, res) {
  ...
  axios
    .get(`http://localhost:5005/title/${title}`)
    .then(function (response) {
      console.log(response.data);
      return res.status(200).json(response.data);
    })
